...
def output_stats():
    ...
    word_freqs_df.to_csv(data_output_dir + 'total_word_frequencies.csv')
    
    links_to_words_df = pd.DataFrame.from_dict(CrawledData.links_to_words, orient='index', columns=['count'])
    links_to_words_df.to_csv(data_output_dir + 'word_count_per_link.csv')

    subdomains_df = pd.DataFrame.from_dict(CrawledData.subdomains, orient='index', columns=['count'])
    subdomains_df.to_csv(data_output_dir + 'subdomain_counts.csv')
